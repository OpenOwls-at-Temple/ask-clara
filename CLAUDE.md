...
- Staging and production run `LLM_PROVIDER=anthropic` only. Non-Anthropic providers (Gemini/DeepSeek) are for local dev and may only ever receive synthetic/test data (`docs/fixtures/`) — never a real resume (see `ai_specs/llm-integration.md` → Privacy & Safety). `LLM_PROVIDER=mock` is a no-network canned provider for Playwright E2E/CI only; it refuses to run outside `ENVIRONMENT=local`.

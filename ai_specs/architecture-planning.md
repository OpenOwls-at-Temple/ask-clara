...
| `last_lead_scan_at` | DateTime (nullable) | Most recent student-triggered job-leads scan; backs the once-per-24h manual-scan limit |
...
| POST | `/api/leads/scan` | (Phase 2) Student-triggered scan of their own profile, at most once per rolling 24h (400 if no target roles — slot not consumed; 429 while the cooldown is active; 503 on failure — slot refunded) |

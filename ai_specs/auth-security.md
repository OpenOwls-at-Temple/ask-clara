...
- Keep access and refresh tokens strictly separated by a `type` claim, enforced on **both** sides. Access tokens carry `type: "access"` and refresh tokens `type: "refresh"`; `decode_access_token` rejects anything that isn't `access` and `decode_refresh_token` rejects anything that isn't `refresh`. This is defense-in-depth on top of the httpOnly-cookie storage: a refresh token can never authenticate a Bearer request even if it leaks out of the cookie. The claim is **required**, not defaulted: a token with no `type` is rejected, so nothing can reach an authenticated route by omission. (A temporary fallback accepted untyped access tokens across the deploy that introduced the claim; it was removed once those tokens aged out of their ~15-min lifetime.) Legacy refresh tokens without a `tv` claim still decode as version 0 — that fallback is unrelated and remains.

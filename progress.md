...
- [ ] **Owner, dashboard:** set the Render health check path to `/api/health` (*Settings → Health Checks*). Currently empty, so Render is falling back to a TCP port probe — which passes as soon as the port binds, even while the app is still starting up and refusing requests. `render.yaml` cannot do this (see next item)

...
- [ ] After `fix/auth-token-type-and-health-probe` deploys: drop the legacy `payload.get("type", "access")` fallback in `decode_access_token` — it is only needed for the ~15 min of untyped access tokens still in flight at deploy, and left in place it permanently accepts a token with no `type` claim
- [x] **Owner, dashboard:** set the Render health check path to `/api/health` (*Settings → Health Checks*) — done 2026-09-06. Previously empty, so Render fell back to a TCP port probe that passed as soon as the port bound, even while the app was still starting up and refusing requests. `render.yaml` cannot do this (see next item)

...
- [ ] After `fix/auth-token-type-and-health-probe` deploys: set Render's health-check path to `/api/health` (owner-managed, Render dashboard), then drop the legacy `payload.get("type", "access")` fallback in `decode_access_token` — it is only needed for the ~15 min of untyped access tokens still in flight at deploy, and left in place it permanently accepts a token with no `type` claim
